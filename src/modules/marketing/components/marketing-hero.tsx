...
/** Premium PropTech gateway hero — Navy + Luxury Gold, no widgets. */
export function MarketingHero({ hero, locale, trustPills }: Props) {
  const arrow = locale === "ar" ? "←" : "→";

  return (
    <section className="ruwaq-landing-hero">
      <div className="ruwaq-landing-hero-glow" aria-hidden />
      ...
      <div className="ruwaq-landing-hero-inner">
        ...
        <p className="mt-6 text-sm text-white/45">{hero.microcopy}</p>

        <ul className="ruwaq-trust-pills ruwaq-trust-pills--luxury mt-8 gap-2.5 sm:mt-10">
          {trustPills.map((pill) => (
            <li key={pill}>{pill}</li>
          ))}
        </ul>
