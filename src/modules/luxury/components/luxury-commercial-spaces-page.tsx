...
export function LuxuryCommercialSpacesPage({ locale }: Props) {
  const copy = getCommercialSpacesCopy(locale);

  return (
    <>
      <section className="lux-section lux-section--white" aria-labelledby="commercial-spaces-title">
        <div className="lux-container grid items-center gap-10 lg:grid-cols-[1.05fr_0.95fr]">
          <div>
            ...
            <h1 id="commercial-spaces-title" className="lux-display mt-4 text-4xl leading-tight text-lux-ink md:text-5xl">
              {copy.hero.title}
            </h1>
          ...
          <div className="relative aspect-[4/5] overflow-hidden rounded-2xl shadow-lux-card sm:aspect-[5/4] lg:aspect-[4/5]">
            <Image
              ...
              alt={locale === "ar" ? "مساحة تستقبل العملاء" : "A space that receives customers"}
              fill
              priority
      ...
      <section id="brief" className="lux-section lux-section--white scroll-mt-24" aria-labelledby="commercial-spaces-cta">
        <div className="lux-container max-w-3xl">
          <h2 id="commercial-spaces-cta" className="lux-display text-3xl leading-tight md:text-4xl">
            {copy.close.title}
          </h2>
